<template>
  <div class="havest_address_container">
    ...
    <form>
      ...
      <button @click.prevent="editAddressInfo">保存地址</button>
    </form>
  </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取地址信息
            getAddressInfo() {
                this.$fetch(this.api.selectAddress, {user_id: this.user_id, id: this.addressId}).then(res => {
                    let addressList = res;
                    this.address = addressList.address;
                    this.houseNumber = addressList.housenumber;
                    this.realName = addressList.realname;
                    this.gender = addressList.gender == '男' ? true : false;
                    this.tel = addressList.tel;
                    this.gender2 = this.gender ? false : true;
                })
            },
            editAddressInfo() {
                if (this.address == '') {
                ...
                this.$post(this.api.UpdateAddressInfo,{
